...
export async function POST(req, { params }) {
  ...
  try {
    ...
    // Enviar siempre email al cliente con el estado actual (se cumple el requisito de notificar al cliente con cada actualización)
    try {
      await sendEmail({
        to: clienteEmail,
      ...
      console.error('Error enviando email al cliente en notificar:', e);
      // no bloqueamos el flujo por fallo en email cliente
    }

    // Enviar email al admin solo cuando el pedido esté en 'pagado' y no haya sido notificado de pago previamente
    let adminSendResult = null;
    if (order.estado === 'pagado' && !order.pagoNotificado) {
      try {
        console.log('notificar endpoint - adminEmail:', adminEmail, 'numeroPedido:', numeroPedido);
  ...
  await Order.updateOne({ _id: order._id }, { $set: { pagoNotificado: true } });
        ...
        console.error('Error enviando email al admin en notificar:', e);
        // no bloqueamos el flujo por fallo en email admin
      }
    }
